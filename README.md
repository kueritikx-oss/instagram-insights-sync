...
| `GOOGLE_SERVICE_ACCOUNT_JSON` | 任意。Service Account JSON の全文。登録されている場合は `GOOGLE_TOKEN_JSON` より優先して Sheets 認証に使う |
...
- 推奨: 長期運用では Service Account を使う。対象スプレッドシートを Service Account の `client_email` に編集者共有し、GitHub Secret `GOOGLE_SERVICE_ACCOUNT_JSON` を登録すると、人間OAuthの `invalid_grant` で止まりにくくなる。
...
- **Google**: Service Account Secret がある場合はそれを優先する。未登録の場合、`token.json` は refresh_token があれば自動更新される。再ログインが必要になったら Mac で 1 回 OAuth し直し、新しい `token.json` で Secret `GOOGLE_TOKEN_JSON` を更新する。
